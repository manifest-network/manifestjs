...
COINS="${COINS:=100000000000000000umfx}"
...
$CHAIN_BIN $CHAIN_GENESIS_CMD add-genesis-account $($CHAIN_BIN keys show -a $(jq -r .genesis[0].name $KEYS_CONFIG) --keyring-backend="test") $COINS,1000000upoa --keyring-backend="test"
...
echo "Creating gentx..."
$CHAIN_BIN $CHAIN_GENESIS_CMD gentx $(jq -r ".genesis[0].name" $KEYS_CONFIG) 1000000upoa --keyring-backend="test" --chain-id $CHAIN_ID
